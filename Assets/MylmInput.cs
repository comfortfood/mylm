...
//		}
	...
	float[] getRawInputs ()
	{
		...
		float pitch = Mathf.Asin (Mathf.Max (-1f, Mathf.Min (1f, 2f * (q.z * q.y - q.w * q.x))));
		float roll = Mathf.Atan2 (2f * (q.z * q.x + q.y * q.w), 1f - 2f * (q.x * q.x + q.y * q.y));
		float yaw = Mathf.Atan2 (2f * (q.z * q.w + q.x * q.y), 1f - 2f * (q.y * q.y + q.w * q.w));
